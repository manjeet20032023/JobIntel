...
const log = debug('jobintel:openwebninja');
...
class OpenWebNinjaService {
  ...
  private requestDelay: number = 1000; // 1 second between requests
  ...
  async searchJobs(params: JobSearchParams): Promise<{
    ...
    return await this.retryRequest(async () => {
      ...
      log(
        `✅ Job search successful: Found ${response.data?.data?.length || 0} jobs`
      );
   ...
   * @returns Salary data
   */
  async querySalary(params: SalaryQueryParams): Promise<{
    status: string;
    data: SalaryData;
  }> {
    if (!this.apiKey) {
    ...
    return await this.retryRequest(async () => {
      ...
      log(
        `✅ Salary data retrieved: ${params.job_title} in ${params.location}`
      );
      return response.data;
   ...
   */
  private validateJobSearchParams(params: JobSearchParams): string | null {
   ...
   */
  private validateSalaryParams(params: SalaryQueryParams): string | null {
...
export default new OpenWebNinjaService();
